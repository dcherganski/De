...
7. **Предпазител** – сигнал „КУПИ“/„ПРОДАЙ“ има само ако ансамбълът мине три прага в walk-forward
   теста: поне 200 прогнози извън извадката; AUC над 0,5 с p < 0,01 (еднопосочен тест на
   Ман–Уитни); Brier по-добър от базовата честота на ръст, известна към момента. Иначе сигналът
   е винаги „ИЗЧАКАЙ“ и прогнозата за посока е само информативна.
...
прогнозата отнема около минута и половина, а първото изтегляне на данните – около минута.
...
Приложените данни в `data/` са дневните свещи от Coinbase за всичките 56 актива от
`watchlist.txt`: по 700 свещи (25.10.2024 – 24.09.2026) за 37 от тях, а по-новите листинги
имат по-кратка история. С тях `predict --offline` работи и без достъп до Coinbase.

### BTC, ETH и SOL

Walk-forward тест за прогноза 1 ден напред, по 499 прогнози на актив (12.05.2025 –
22.09.2026):
...
### Целият списък: 54 прогнози към 24.09.2026

`python -m crypto_bot predict --offline --events-file data/events_fomc.csv` дава 54
прогнози. DRV и BILL са пропуснати, защото са листнати скоро и имат 119 и 143 затворени
свещи при минимум 230.

| | Брой |
|---|---:|
| Сигнал „ИЗЧАКАЙ“ | 53 |
| Сигнал „ПРОДАЙ“ (в long-only режим: излез от позиция) | 1 – DASH |
| Сигнал „КУПИ“ | 0 |
| Вероятност за ръст под 50% | 48 |
| С поне 200 прогнози извън извадката | 48 |
| Минали теста за предимство (виж по-долу) | 1 – DASH |

Прочети тези числа внимателно:

- **Посоката остава на нивото на монетата.** Медианният AUC на ансамбъла е 0,495
  (от 0,425 до 0,587), а AUC > 0,5 имат 24 от 54 актива – колкото се очаква и при чист
  случай. Старият предпазител (Brier skill > 0 и AUC > 0,5) пускаше 17 актива – около една
  трета, колкото минават и по случайност; при HYPE „предимството“ идваше от 31 прогнози,
  при GFI от AUC 0,502.
- **Тестът за предимство иска доказателство, не съвпадение.** Три прага, всички
  задължителни: поне 200 прогнози извън извадката (48 актива ги имат); AUC над 0,5 с
  p < 0,01 по еднопосочен тест на Ман–Уитни (при 499 прогнози това значи AUC ≳ 0,56);
  Brier по-добър от базовата честота на ръст, известна към момента (9 актива). Минава го
  само DASH (AUC 0,561, p = 0,009, Brier skill спрямо базата +0,012), и то на ръба: при
  54 актива и p < 0,01 се очаква около половин лъжливо положителен резултат, така че и
  DASH може да е случайност. Най-близо след него са SEI (p = 0,029) и MET (p = 0,041, но
  само 136 прогнози). Проверка на самия тест: върху синтетични данни със скрита
  авторегресия 0,5 той дава p < 0,001, а върху случайно блуждаене – p = 0,19 и 0,66.
- **Защо почти всичко е под 50%.** Периодът на теста е пазар, който слиза: медианната
  честота на дни с ръст е 45,7%. Моделите са научили този наклон, не конкретна закономерност.
- **Стратегиите „бият“ купи-и-дръж най-вече като стоят в кеш.** При 17-те актива, минали
  стария праг, симулацията е в позиция 1–48% от времето, а 6 от тях са на загуба (TAO, PEPE,
  APT, SEI, SKY, HYPE). При DASH тя дава +83% срещу +132% за купи-и-дръж, със Sharpe 0,92 и
  максимална просадка 36%.
- **Коридорът на волатилността държи за всички.** В 68%-ния коридор попадат медианно 68,0%
  от цените (57,6% – 72,0%), а в 95%-ния – 94,5% (88,7% – 95,8%). Най-слабо калибрирани са
  активите с кратка история и резки движения.
- **Вероятността и очакваната промяна се оценяват поотделно.** Вероятността е делът на
  случаите с ръст (класификатор), а очакваната промяна е средният доход (регресия). При
  изкривено разпределение, например много малки ръстове и няколко големи спада, двете сочат в
  различни посоки. Така е при 9 актива (BTC, ZEC, LTC, BCH, HBAR, PENGU, ENA, SPX, CAKE).
  Сигналът следва вероятността, а разминаването е още един знак, че посоката е несигурна.

Прогнозата е за затварянето на 25.09.2026 г. и остарява с всяка нова свещ. Пусни я
отново с `fetch` и `predict`, за да я обновиш.
